...
// 📝 Placeholder for future events – Currently no events
...
function FutureEventDetails() {
  ...
  // Type-safe check for event based on URL param
  const event = id ? futureEventDetails[id as keyof typeof futureEventDetails] : null;

  // 🚫 No event available – User-friendly message
  if (!event) {
    return (
      <div className="min-h-screen pt-24 pb-12">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-3xl font-bold mb-4 text-gray-800">🚧 No Event Available</h1>
          <p className="mb-2 text-lg text-gray-600">
            We're currently working behind the scenes to bring you some exciting events.
          </p>
          <p className="mb-2 text-lg text-gray-600">
            Stay tuned for upcoming hackathons, workshops, and tech fests — curated just for you!
          </p>
          <p className="mb-6 text-lg text-gray-600">
            In the meantime, feel free to explore our past events and get inspired.
          </p>
          <button
  ...
  // 🔽 This section will only render when future events exist
  // return (
  //   <div className="min-h-screen pt-24 pb-12">
  //     <div className="container mx-auto px-4">
  //       {/* Back Navigation */}
  //       <button onClick={() => navigate('/future-events')} className="flex items-center mb-4 text-blue-600 hover:text-blue-800">
  ...
  //           />
  //           <button
  //             onClick={() => setCurrentSlide((currentSlide - 1 + event.images.length) % event.images.length)}
  //             className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white p-2 rounded-full shadow hover:bg-gray-100"
  //           >
  //             <ChevronLeft />
  //           </button>
  //           <button
  //             onClick={() => setCurrentSlide((currentSlide + 1) % event.images.length)}
  ...
  //       {/* Additional event details like schedule, prizes, etc., can go here in future */}
  //     </div>
  //   </div>
  // );
}
